...
/* 串行Flsh的片选GPIO端口  */
#define SF_RCC_CS 			RCC_AHBPeriph_GPIOB
...
void sf_ConfigGPIO(void)
{
	...
	*/
	...
	/* 使能GPIO 时钟 */
	RCC_AHBPeriphClockCmd(SF_RCC_CS, ENABLE);
...
void bsp_InitSFlash(void)
{
	...
	bsp_spiWrite1(FLASH_SPI,CMD_DISWR);		/* 发送禁止写入的命令,即使能软件写保护 */
...
void sf_EraseSector(uint32_t _uiSectorAddr)
{
	...
	/* 擦除扇区操作 */
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_SE);								/* 发送擦除命令 */
	bsp_spiWrite1(FLASH_SPI,(_uiSectorAddr & 0xFF0000) >> 16);	/* 发送扇区地址的高8bit */
	bsp_spiWrite1(FLASH_SPI,(_uiSectorAddr & 0xFF00) >> 8);		/* 发送扇区地址中间8bit */
	bsp_spiWrite1(FLASH_SPI,_uiSectorAddr & 0xFF);				/* 发送扇区地址低8bit */
	sf_SetCS(1);									/* 禁能片选 */
...
void sf_EraseChip(void)
{
	...
	/* 擦除扇区操作 */
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_BE);							/* 发送整片擦除命令 */
	sf_SetCS(1);									/* 禁能片选 */
...
void sf_PageWrite(uint8_t * _pBuf, uint32_t _uiWriteAddr, uint16_t _usSize)
{
	...
	if (g_tSF.ChipID == SST25VF016B_ID)
	{
		...
		sf_SetCS(0);									/* 使能片选 */
		bsp_spiWrite1(FLASH_SPI,CMD_AAI);							/* 发送AAI命令(地址自动增加编程) */
		bsp_spiWrite1(FLASH_SPI,(_uiWriteAddr & 0xFF0000) >> 16);	/* 发送扇区地址的高8bit */
		bsp_spiWrite1(FLASH_SPI,(_uiWriteAddr & 0xFF00) >> 8);		/* 发送扇区地址中间8bit */
		bsp_spiWrite1(FLASH_SPI,_uiWriteAddr & 0xFF);				/* 发送扇区地址低8bit */
		bsp_spiWrite1(FLASH_SPI,*_pBuf++);							/* 发送第1个数据 */
		bsp_spiWrite1(FLASH_SPI,*_pBuf++);							/* 发送第2个数据 */
		sf_SetCS(1);									/* 禁能片选 */
		...
		for(i=0; i<_usSize/2; i++)
		{
			sf_SetCS(0);								/* 使能片选 */
			bsp_spiWrite1(FLASH_SPI,CMD_AAI);						/* 发送AAI命令(地址自动增加编程) */
			bsp_spiWrite1(FLASH_SPI,*_pBuf++);						/* 发送数据 */
			bsp_spiWrite1(FLASH_SPI,*_pBuf++);						/* 发送数据 */
			sf_SetCS(1);								/* 禁能片选 */
			sf_WaitForWriteEnd();						/* 等待串行Flash内部写操作完成 */
		}
		...
		/* 进入写保护状态 */
		sf_SetCS(0);
		bsp_spiWrite1(FLASH_SPI,CMD_DISWR);
		sf_SetCS(1);
	...
	else	/* for MX25L1606E 、 W25Q64BV */
	{
		for (j = 0; j < _usSize / 256; j++)
		{
			...
			sf_SetCS(0);									/* 使能片选 */
			bsp_spiWrite1(FLASH_SPI,0x02);								/* 发送AAI命令(地址自动增加编程) */
			bsp_spiWrite1(FLASH_SPI,(_uiWriteAddr & 0xFF0000) >> 16);	/* 发送扇区地址的高8bit */
			bsp_spiWrite1(FLASH_SPI,(_uiWriteAddr & 0xFF00) >> 8);		/* 发送扇区地址中间8bit */
			bsp_spiWrite1(FLASH_SPI,_uiWriteAddr & 0xFF);				/* 发送扇区地址低8bit */
...
void sf_ReadBuffer(uint32_t _uiReadAddr, uint8_t * _pBuf, uint32_t _uiSize)
{
	...
	/* 擦除扇区操作 */
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_READ);							/* 发送读命令 */
	bsp_spiWrite1(FLASH_SPI,(_uiReadAddr & 0xFF0000) >> 16);	/* 发送扇区地址的高8bit */
	bsp_spiWrite1(FLASH_SPI,(_uiReadAddr & 0xFF00) >> 8);		/* 发送扇区地址中间8bit */
	bsp_spiWrite1(FLASH_SPI,_uiReadAddr & 0xFF);				/* 发送扇区地址低8bit */
	while (_uiSize--)
	{
		*_pBuf++ = bsp_spiRead1(FLASH_SPI);			/* 读一个字节并存储到pBuf，读完后指针自加1 */
	}
	sf_SetCS(1);									/* 禁能片选 */
}
...
static uint8_t sf_CmpData(uint32_t _uiSrcAddr, uint8_t *_ucpTar, uint32_t _uiSize)
{
	...
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_READ);							/* 发送读命令 */
	bsp_spiWrite1(FLASH_SPI,(_uiSrcAddr & 0xFF0000) >> 16);		/* 发送扇区地址的高8bit */
	bsp_spiWrite1(FLASH_SPI,(_uiSrcAddr & 0xFF00) >> 8);		/* 发送扇区地址中间8bit */
	bsp_spiWrite1(FLASH_SPI,_uiSrcAddr & 0xFF);					/* 发送扇区地址低8bit */
	while (_uiSize--)
	{
		/* 读一个字节 */
		ucValue = bsp_spiRead1(FLASH_SPI);
		if (*_ucpTar++ != ucValue)
		{
			sf_SetCS(1);
			return 1;
...
static uint8_t sf_AutoWritePage(uint8_t *_ucpSrc, uint32_t _uiWrAddr, uint16_t _usWrLen)
{
	...
	sf_ReadBuffer(_uiWrAddr, s_spiBuf, _usWrLen);
	...
	{
		/* 先将整个扇区的数据读出 */
		sf_ReadBuffer(uiFirstAddr, s_spiBuf, g_tSF.PageSize);
...
uint32_t sf_ReadID(void)
{
	...
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_RDID);				/* 发送读ID命令 */
	id1 = bsp_spiRead1(FLASH_SPI);					/* 读ID的第1个字节 */
	id2 = bsp_spiRead1(FLASH_SPI);					/* 读ID的第2个字节 */
	id3 = bsp_spiRead1(FLASH_SPI);					/* 读ID的第3个字节 */
	sf_SetCS(1);									/* 禁能片选 */
...
void sf_ReadInfo(void)
{
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			...
			case MX25L12835E_ID:
			{				
				strcpy(g_tSF.ChipName, "MX25L128");
				g_tSF.TotalSize = 16 * 1024 * 1024;	/* 总容量 = 16M */
				g_tSF.PageSize = 4 * 1024;			/* 页面大小 = 4K */
			}
			break;
...
{
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_WREN);								/* 发送命令 */
	sf_SetCS(1);									/* 禁能片选 */
}
...
static void sf_WriteStatus(uint8_t _ucValue)
{
	...
	if (g_tSF.ChipID == SST25VF016B_ID)
	{
		/* 第1步：先使能写状态寄存器 */
		sf_SetCS(0);									/* 使能片选 */
		bsp_spiWrite1(FLASH_SPI,CMD_EWRSR);							/* 发送命令， 允许写状态寄存器 */
		sf_SetCS(1);									/* 禁能片选 */

		/* 第2步：再写状态寄存器 */
		sf_SetCS(0);									/* 使能片选 */
		bsp_spiWrite1(FLASH_SPI,CMD_WRSR);							/* 发送命令， 写状态寄存器 */
		bsp_spiWrite1(FLASH_SPI,_ucValue);							/* 发送数据：状态寄存器的值 */
		sf_SetCS(1);									/* 禁能片选 */
	}
	else
	{
...
{
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(FLASH_SPI,CMD_RDSR);							/* 发送命令， 读状态寄存器 */
	while((bsp_spiRead1(FLASH_SPI) & WIP_FLAG) == SET)	/* 判断状态寄存器的忙标志位 */
	{
		//喂狗
		bsp_iwdg_clear();
	}

...
*	修改记录 :
*		版本号  日期       作者    说明
*		V1.0    2013-02-01 ymh  正式发布
*
*	Copyright (C), 2013-2014, YMH
*
*********************************************************************************************************
*/
#include "bsp.h"
...
#if RS485_EN == 1
	#define RCC_RS485_TXEN					RCC_AHBPeriph_GPIOC 
	#define PORT_RS485_TXEN             	GPIOC
	#define PIN_RS485_TXEN              	GPIO_Pin_4
	#define RS485_TXEN() 					PORT_RS485_TXEN->BSRRL = PIN_RS485_TXEN
	#define RS485_RXEN() 					PORT_RS485_TXEN->BSRRH = PIN_RS485_TXEN
#endif
...
static uint16_t UartGetRxCount(UART_T *_pUart);
...
*********************************************************************************************************
*	函 数 名: comGetBytes
*	功能说明: 从串口缓冲区读取多个字节，非阻塞。无论有无数据均立即返回
*	形    参: _ucPort: 端口号(COM1 - COM6)
*			  _pByte: 接收到的数据存放在这个地址
*	返 回 值: 0 表示无数据, >0 表示读取到有效字节
*********************************************************************************************************
*/
uint16_t comGetBytes(COM_PORT_E _ucPort, uint8_t *_pBuf, uint16_t _usSize)
{
	...
	memset(_pBuf, 0, _usSize);
	count = comGetRxCount(_ucPort);
	if(count > _usSize)
	{
		for(i=0; i<_usSize; i++)
		{
			comGetChar(_ucPort, _pBuf+i);			
		}
		return _usSize;
	}
	else
...
uint16_t comGetRxCount(COM_PORT_E _ucPort)
{
	...
	return UartGetRxCount(pUart);
}

/*
*********************************************************************************************************
*	函 数 名: comGetRxTimeOut
*	功能说明: 串口接收字符超时
*	形    参: _ucPort: 端口号(COM1 - COM6)
*	返 回 值: 数据个数
*********************************************************************************************************
...
//	if (_ucPort == COM1)
//	{
//		return bsp_timer_check(TMR_RX1);
//	}
//	else if (_ucPort == COM2)
//	{
//		return bsp_timer_check(TMR_RX2);
//	}
//	else if (_ucPort == COM3)
//	{
//		return bsp_timer_check(TMR_RX3);
//	}
...
void comClearRxFifo(COM_PORT_E _ucPort)
{
	...
	if (_ucPort == COM1)
	{
		//bsp_timer_stop(TMR_RX1);
	}
	else if (_ucPort == COM2)
	{
		//bsp_timer_stop(TMR_RX2);
	}
	else if (_ucPort == COM3)
	{
		//bsp_timer_stop(TMR_RX3);
	}		
}
...
*/
	...
	{
		 //bsp_timer_star_once(TMR_RX3, TM_30MS);
	}
#endif
	
/*
*********************************************************************************************************
*	函 数 名: COM1_ReciveNew
*	功能说明: 接收到新的数据
*	形    参: _byte 接收到的新数据
*	返 回 值: 无
*********************************************************************************************************
*/
	void COM1_ReciveNew(uint8_t _byte)
	{
		 //bsp_timer_star_once(TMR_RX1, TM_100MS);
	}
...
*********************************************************************************************************
*/
	void COM2_ReciveNew(uint8_t _byte)
	{
		// bsp_timer_star_once(TMR_RX2, TM_10MS);
	}	
...
static void UartVarInit(void)
{
	...
	g_tUart1.ReciveNew   = COM1_ReciveNew;			/* 接收到新数据后的回调函数 */
#endif
	...
	g_tUart2.ReciveNew   = COM2_ReciveNew;;			/* 接收到新数据后的回调函数 */
#endif
...
void InitHardUart3(void)
{
	...
	/* 第2步： 配置串口硬件参数 */
  if(g_tVar.ucBAD2 == 1)		USART_InitStructure.USART_BaudRate = 1200;	/* 波特率 */
	else if(g_tVar.ucBAD2 == 2)	USART_InitStructure.USART_BaudRate = 2400;
	else if(g_tVar.ucBAD2 == 3)	USART_InitStructure.USART_BaudRate = 4800;
	else if(g_tVar.ucBAD2 == 4)	USART_InitStructure.USART_BaudRate = 9600;
	else if(g_tVar.ucBAD2 == 5)	USART_InitStructure.USART_BaudRate = 19200;
	else if(g_tVar.ucBAD2 == 6)	USART_InitStructure.USART_BaudRate = 38400;
	else if(g_tVar.ucBAD2 == 7)	USART_InitStructure.USART_BaudRate = 57600;		
	else if(g_tVar.ucBAD2 == 8)	USART_InitStructure.USART_BaudRate = 115200;
	else 						USART_InitStructure.USART_BaudRate = 2400;
...
/*
*********************************************************************************************************
*	函 数 名: UartGetRxCount
*	功能说明: 从串口接收缓冲区读取新数据个数
*	形    参: _pUart : 串口设备
...
uint16_t comRxPrintfHex(COM_PORT_E _ucPort, uint8_t *_pBuf, uint16_t _usSize)
{
	...
	len = comGetBytes( _ucPort, _pBuf, _usSize);
	if(len)
	{
		printf("\r\n");
		for(i=0; i<len; i++)
...
/*
*********************************************************************************************************
*	函 数 名: comTxPrintfHex
*	功能说明: 打印串口发送的十六进制数据
*	形    参: 
*	返 回 值: 
*********************************************************************************************************
*/
uint16_t comTxPrintfHex(COM_PORT_E _ucPort, uint8_t *_ucaBuf, uint16_t _usLen)
{
	uint16_t i;
